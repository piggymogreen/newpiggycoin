...
const string strMessageMagic = "newpiggycoin Signed Message:\n";
...
{
    int64_t nSubsidy = 23700 * COIN;
        if(pindexBest->nHeight == 2)
        {
            nSubsidy = 265353096 * COIN;
        }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (nStakeReward < (nCalculatedStakeReward / 2))
            return DoS(100, error("ConnectBlock() : coinstake pays too little(actual=%"PRId64" vs calculated=%"PRId64")", nStakeReward, nCalculatedStakeReward));
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "newpiggycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The new Piggycoin is here";
        CTransaction txNew;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x5baa2de3e46c0e728b7751e2d194f16538c6a6595cb0d57cda27259d9d7db8e9"));
        block.print();
